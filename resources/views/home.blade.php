        ...
        @auth
            @if(!$sudah_lapor_hari_ini)
                <div class="p-4 mb-4 text-sm text-yellow-800 rounded-lg bg-yellow-50 dark:bg-gray-800 dark:text-yellow-300" role="alert">
                    <span class="font-medium flex"><x-lucide-alert-circle class="w-5 h-5 mr-1" /> Sepertinya kamu belum lapor hari ini. Yuk mulai setor laporan harian <a href="{{ route('laporan.create') }}" class="ml-1 text-[#007dd9]">disini</a></span>
                </div>
            @endif
        @endauth
            ...
            @foreach ($list_jadwal_sholat as $jadwal)
                <li class="flex items-center gap-4 py-2 relative before:border-l before:border-[#e3e3e0] dark:before:border-[#3E3E3A] before:top-1/2 before:bottom-0 before:left-[0.4rem] before:absolute">
                    <span class="relative py-1 bg-white dark:bg-[#161615]">
                        <span class="flex items-center justify-center rounded-full bg-[#FDFDFC] dark:bg-[#161615] shadow-[0px_0px_1px_0px_rgba(0,0,0,0.03),0px_1px_2px_0px_rgba(0,0,0,0.06)] w-3.5 h-3.5 border @if($jadwal['is'])border-[#007dd9] breathing-border @else dark:border-[#3E3E3A] border-[#e3e3e0] @endif">
                            <span class="rounded-full @if($jadwal['is']) bg-[#007dd9] breathing-dot @else bg-[#dbdbd7] dark:bg-[#3E3E3A] @endif w-1.5 h-1.5"></span>
                        </span>
                    </span>
